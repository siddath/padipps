...
export function setupPacks({download, announce, onChange, beforeSwitch}) {
  ...
  function page() {
    ...
    <section class="section"><h2>Explore the included library</h2><div class="row-list"><article class="session-row"><div class="row-main"><h3>Engineering &amp; product design</h3><p>Eight ordered AI modules, Kafka consumer lag, and the existing engineering and product-design exercises. Choose the new pack to open its own notebook.</p></div><button class="btn" data-pack-action="engineering">Preview engineering pack</button></article></div><p class="fine">Choosing this pack opens its own notebook. Your current work stays with its original pack. <a href="packs/engineering.json" download>Download the engineering pack</a> · <a href="https://github.com/siddath/padipps/tree/main/materials/engineering" target="_blank" rel="noopener noreferrer">Exercise materials ↗</a></p><p class="fine">Already studying the 31-lesson pack? Keep using it, or reopen its notebook below. Switching to the expanded pack keeps those records separate.</p><button class="btn quiet" data-pack-action="engineering-previous">Preview previous engineering pack</button></section>
  ...
  document.addEventListener('click', async event => {
    ...
    switch(button.dataset.packAction){
      ...
      case 'engineering':
      case 'engineering-previous':
        pending=null;useStarter=false;
        try { const response=await fetch(new URL(button.dataset.packAction==='engineering-previous'?'./packs/engineering-v1.json':'./packs/engineering.json',import.meta.url));if(!response.ok)throw Error('Engineering pack is unavailable.');const parsed=parsePack(await response.text());if(!parsed.ok)throw Error(parsed.errors.join(' '));pending=parsed.pack;message='Engineering preview ready. Review it before opening a separate notebook.'; }
