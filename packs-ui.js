...
export function setupPacks({download, announce, onChange, beforeSwitch}) {
  ...
  function page() {
    return `<div class="page-top"><div><p class="page-kicker">Your material, your notebook</p><h1>Bring a study pack.</h1><p class="intro">Import a JSON file from your own repository or folder. Preview the lessons before opening their notebook.</p></div></div>
    ...
    <section class="next-session pack-current"><span class="pill">Current pack</span><h2>${esc(PACK?.title || 'No pack loaded')}</h2><p>${esc(PACK?.description || 'Import a pack below to begin.')}</p><p class="fine">${PACK?.sessions.length || 0} lesson${PACK?.sessions.length===1?'':'s'} · ${PACK?.tracks.length || 0} track${PACK?.tracks.length===1?'':'s'}${PACK ? ` · ${esc(PACK.id)}` : ''}</p><div class="actions"><a class="button-link primary" href="#practice">Open practice →</a>${PACK ? '<button class="btn" data-pack-action="export">Download current pack</button>' : ''}</div></section>
    <section class="notebook-section section"><h2>Import your material</h2><p>Import a JSON study pack, preview its lessons, then choose whether to use it. Your file stays in this browser. Each pack keeps a separate notebook and focus record.</p><label class="field" for="pack-import">Import study pack JSON</label><input id="pack-import" type="file" accept=".json,application/json"><p class="fine">Up to 2 MB. Keep a copy of every pack you use so you can reopen its notebook later. Download the current pack before replacing an edited curriculum. <a href="packs/starter.json" download>Download sample template</a>.</p><button class="text-button" data-pack-action="starter">Preview the sample pack</button><p class="feedback" id="pack-message" role="status" tabindex="-1">${esc(message)}</p>
    ${pending ? `<div class="note pack-preview"><p class="page-kicker">Preview · ${pending.sessions.length} lesson${pending.sessions.length===1?'':'s'} · ${pending.tracks.length} track${pending.tracks.length===1?'':'s'}</p><h3>${esc(pending.title)}</h3><p>${esc(pending.description)}</p><details><summary>See the lessons</summary><ul>${pending.sessions.map(s=>`<li>${esc(s.title)} <span class="fine">· ${esc(s.category)}</span></li>`).join('')}</ul></details><p class="fine">${PACK?.id===pending.id && JSON.stringify(PACK)!==JSON.stringify(pending) ? 'This is a new content revision. It opens a separate notebook; retain the old pack file to reopen its history.' : 'Your existing notebook stays under its current pack ID. A running focus timer must be paused before switching.'}</p><div class="actions"><button class="btn primary" data-pack-action="use">Use this pack →</button><button class="text-button" data-pack-action="cancel">Cancel</button></div></div>` : ''}</section>
    <section class="notebook-portable"><h2>Keep your curriculum in your own repository.</h2><p>Build a pack locally from a repository you control, validate the JSON, then import it here. Repository authentication stays in your own Git tools. This app does not clone repositories or sync changes.</p><p>Imported packs are teaching material from their author. Validation checks their structure, not the accuracy of their ideas.</p><a href="https://github.com/siddath/padipps/blob/main/PACKS.md" target="_blank" rel="noopener noreferrer">Read the pack format ↗</a> · <a href="https://github.com/siddath/padipps/blob/main/docs/CONTENT_PIPELINE.md" target="_blank" rel="noopener noreferrer">Build a local content pipeline ↗</a></section>`;
  }
  ...
  document.addEventListener('click', async event => {
    ...
    switch(button.dataset.packAction){
      ...
      case 'starter':
        try { const response=await fetch(STARTER_PACK_URL);if(!response.ok)throw Error('Starter file is unavailable.');const parsed=parsePack(await response.text());if(!parsed.ok)throw Error(parsed.errors.join(' '));pending=parsed.pack;useStarter=true;message='Sample preview ready. Your current notebook stays separate.'; }
