...
function sessionPage(id) {
  ...
  return `<div class="session-heading"><div><p class="eyebrow">${esc(session.category || 'Practice')}</p><h1>${esc(session.title)}</h1></div><div class="session-tools"><button class="text-button" data-action="session-note" data-session-id="${esc(session.id)}">Write a note</button><button class="btn quiet" data-action="pause-leave">Save & leave</button></div></div><div class="progress-track" aria-hidden="true"><span class="progress-ink"></span></div><ol class="steps" aria-label="Session stages">${steps.map((step, index) => `<li data-stage="${index}" ${index === draft.stage ? 'aria-current="step"' : ''} class="${index < draft.stage ? 'done' : ''}">${index + 1}. ${step}</li>`).join('')}</ol><div class="lesson-layout"><section class="lesson-pane">${body}</section>${sourceAside(session,draft)}</div>`;
}
...
function render(focus = true, kind = 'view') {
  ...
  storageUI(); if (focus) { $('#main').focus({ preventScroll:true }); window.scrollTo(0, 0); } bindForms();
